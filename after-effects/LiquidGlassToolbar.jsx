...
(function liquidGlassToolbar7() {
    ...
    slider("Frost", 5); slider("Refract", 320); slider("Float", 0);
    ...
    addGauss(height, 18, true);            // tighter slope = thinner, Apple-style edge band
    ...
    if (ccg) {
        ...
        var sft = pn(ccg, ["Softness"]);     if (sft) { try { sft.setValue(5); } catch (e) {} }
        var hgt = pn(ccg, ["Height"]);       if (hgt) { try { hgt.setValue(140); } catch (e) {} }
    ...
    try {
        comp.layer("Controls").effect("Refract").property(1).setValue(distortType === "spherize" ? 90 : 320);
    } catch (e) {}
    ...
    alert("Liquid Glass Toolbar — build 10 (clean edge refraction) ✨\n\n" +
          ">>> Distortion: " + (distortType ? distortType.toUpperCase() : "NONE") + " <<<\n" +
          (distortType === "ccglass"
            ? "CC Glass loaded — edge refraction follows the null, clean.\n\n"
            : "*** CC Glass is NOT available on this install, so it fell back\n" +
              "    to center-magnify (Spherize). Tell me and I'll switch the\n" +
              "    approach — clean edge refraction needs CC Glass. ***\n\n") +
          "• Drag the 'Glass' null to move the toolbar.\n" +
          "• Refraction is a DUPLICATE of the 'Glass Content' comp (that's\n" +
          "  why it's clean and follows the pill). Put your own layers\n" +
          "  INSIDE the 'Glass Content' comp.\n" +
          "• Controls: Refract = edge distortion (try 200-500), Frost = blur.");
