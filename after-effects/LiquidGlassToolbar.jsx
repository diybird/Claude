/**********************************************************************
 * Liquid Glass TOOLBAR — null-driven, EDGE refraction.  (build 9)
 * Based on the adjustment-layer version you liked, now with real
 * edge distortion (Displacement Map + capsule normal map) PLUS a
 * cranked Spherize for extra body distortion.
 *
 * RUN:  File › Scripts › Run Script File…  ›  this file
 *
 * • Drag the "Glass" null to move the toolbar.
 * • "Glass / Frost" is an ADJUSTMENT layer: it refracts + blurs the
 *   layers BELOW it. Put your content on layers below it (above DEMO).
 * • Controls: Refract = distortion strength, Frost = blur, Float = drift.
 *********************************************************************/

(function liquidGlassToolbar9() {

    var W = 900, H = 440, FPS = 30, DUR = 6;
    var CX = W / 2, CY = H / 2;
    var PW = 436, PH = 132, PR = 66;
    var pTopY = CY - PH / 2, pBotY = CY + PH / 2;
    var START = [W / 2, 150];
    var distortType = null, hasDisp = false;

    app.beginUndoGroup("Build Liquid Glass Toolbar 9");

    // ===================== helpers =====================
    function setExpr(p, e) { if (p) p.expression = e; }
    function pos(L) { return L.property("ADBE Transform Group").property("ADBE Position"); }
    function setOp(L, v) { L.property("ADBE Transform Group").property("ADBE Opacity").setValue(v); }
    function setPos(L, p) { pos(L).setValue(p); }
    function follow(L, ox, oy) { setExpr(pos(L), 'thisComp.layer("Glass").transform.position + [' + (ox||0) + ',' + (oy||0) + ']'); }
    function roundedRectShape(cx, cy, w, h, r) {
        var k = r * 0.5523, l = cx-w/2, rt = cx+w/2, t = cy-h/2, b = cy+h/2, s = new Shape();
        s.vertices = [[l+r,t],[rt-r,t],[rt,t+r],[rt,b-r],[rt-r,b],[l+r,b],[l,b-r],[l,t+r]];
        s.inTangents  = [[-k,0],[0,0],[0,-k],[0,0],[k,0],[0,0],[0,k],[0,0]];
        s.outTangents = [[0,0],[k,0],[0,0],[0,k],[0,0],[-k,0],[0,0],[0,-k]];
        s.closed = true; return s;
    }
    function ellipseShapeObj(cx, cy, rx, ry) {
        var kx = rx*0.5523, ky = ry*0.5523, s = new Shape();
        s.vertices=[[cx,cy-ry],[cx+rx,cy],[cx,cy+ry],[cx-rx,cy]];
        s.inTangents=[[-kx,0],[0,-ky],[kx,0],[0,ky]]; s.outTangents=[[kx,0],[0,ky],[-kx,0],[0,-ky]];
        s.closed=true; return s;
    }
    function addMaskTo(L, comp2, shape, feather, mode) {
        var m = L.property("ADBE Mask Parade").addProperty("ADBE Mask Atom");
        m.property("ADBE Mask Shape").setValue(shape);
        if (feather != null) m.property("ADBE Mask Feather").setValue([feather, feather]);
        if (mode) m.maskMode = mode; return m;
    }
    function prop(fx, name, idx) { var p=null; try{p=fx.property(name);}catch(e){} if(!p){try{p=fx.property(idx);}catch(e2){}} return p; }
    function pn(fx, names){ for(var i=0;i<names.length;i++){try{var p=fx.property(names[i]); if(p)return p;}catch(e){}} return null; }
    function capsulePathExpr(w, h, r) {
        return [
            'var n=thisComp.layer("Glass").transform.position;',
            'var w='+w+',h='+h+',r='+r+';',
            'var cx=n[0],cy=n[1];',
            'var l=cx-w/2,rt=cx+w/2,t=cy-h/2,b=cy+h/2,k=r*0.5523;',
            'var pts=[[l+r,t],[rt-r,t],[rt,t+r],[rt,b-r],[rt-r,b],[l+r,b],[l,b-r],[l,t+r]];',
            'var inT=[[-k,0],[0,0],[0,-k],[0,0],[k,0],[0,0],[0,k],[0,0]];',
            'var outT=[[0,0],[k,0],[0,0],[0,k],[0,0],[-k,0],[0,0],[0,-k]];',
            'createPath(pts,inT,outT,true);'
        ].join('\n');
    }
    function addGauss(L, amt, repeatEdge) {
        var fx; try { fx = L.property("ADBE Effect Parade").addProperty("ADBE Gaussian Blur 2"); }
        catch (e) { fx = L.property("ADBE Effect Parade").addProperty("ADBE Gaussian Blur"); }
        if (amt != null) fx.property(1).setValue(amt);
        if (repeatEdge) { try { fx.property(3).setValue(1); } catch (e2) {} } return fx;
    }
    function addRamp(L, p1, c1, p2, c2) {
        var fx = L.property("ADBE Effect Parade").addProperty("ADBE Ramp");
        fx.property("ADBE Ramp-0001").setValue(p1); fx.property("ADBE Ramp-0002").setValue(c1);
        fx.property("ADBE Ramp-0003").setValue(p2); fx.property("ADBE Ramp-0004").setValue(c2); return fx;
    }
    function addEffect(L, mn) { try { return L.property("ADBE Effect Parade").addProperty(mn); } catch (e) { return null; } }
    function setP(fx, idx, val) { try { fx.property(idx).setValue(val); } catch (e) {} }

    // ============================================================
    // NORMAL MAP precomp: capsule edge encoded as R=horiz slope, G=vert slope
    // (Emboss gives the signed gradient; Shift Channels routes it to R / G.)
    // ============================================================
    var nrm = app.project.items.addComp("Glass Normal", W, H, 1, DUR, FPS);
    function embossAxis(name, dir, takeRed, takeGreen, takeBlue) {
        var L = nrm.layers.addSolid([1, 1, 1], name, W, H, 1);
        addMaskTo(L, nrm, roundedRectShape(CX, CY, PW, PH, PR), 1);
        addGauss(L, 24, true);
        var em = addEffect(L, "ADBE Emboss");
        if (em) { setP(em, 1, dir); setP(em, 2, 7); setP(em, 3, 1); setP(em, 4, 0); } // Direction, Relief, Contrast, Blend
        var sc = addEffect(L, "ADBE Shift Channels");
        if (sc) { setP(sc, 2, takeRed); setP(sc, 3, takeGreen); setP(sc, 4, takeBlue); } // Red/Green/Blue From
        return L;
    }
    // 10 = "Off" (0), 2 = "Red", 3 = "Green"
    embossAxis("Hx", 0,  2, 10, 10);                 // horizontal slope -> Red
    var hy = embossAxis("Hy", 90, 10, 3, 10);        // vertical slope   -> Green
    try { hy.blendingMode = BlendingMode.ADD; } catch (e) {}

    // ============================================================
    // MAIN comp
    // ============================================================
    var comp = app.project.items.addComp("Liquid Glass Toolbar", W, H, 1, DUR, FPS);
    comp.bgColor = [0, 0, 0]; comp.openInViewer();
    function solid(name, color) { return comp.layers.addSolid(color, name, W, H, 1); }
    function addMask(L, shape, feather, mode) { return addMaskTo(L, comp, shape, feather, mode); }

    // control nulls FIRST
    var controls = comp.layers.addNull(DUR); controls.name = "Controls";
    setPos(controls, [26, 26]);
    var cFx = controls.property("ADBE Effect Parade");
    function slider(name, val) { var fx = cFx.addProperty("ADBE Slider Control"); fx.name = name; fx.property(1).setValue(val); return fx; }
    slider("Frost", 6); slider("Refract", 150); slider("Float", 0);

    var glass = comp.layers.addNull(DUR); glass.name = "Glass";
    setPos(glass, START);
    setExpr(pos(glass),
        'var f=thisComp.layer("Controls").effect("Float")("Slider");\n' +
        '(f>0)?[' + (W/2) + '+Math.cos(time*0.6)*' + (W*0.26) + ',' + START[1] + ']:value;');

    // ----- DEMO content (replace with your own; keep below Glass/Frost) -----
    solid("DEMO bg", [0.02, 0.02, 0.02]);
    var green = solid("DEMO green card", [0.3, 0.6, 0.28]);
    addMask(green, roundedRectShape(450, 560, 840, 760, 78), 1.5);
    addRamp(green, [450, 175], [0.42, 0.74, 0.34], [450, 470], [0.15, 0.40, 0.16]);
    var txt = comp.layers.addText("LIQUID GLASS");
    (function () { var td = txt.property("ADBE Text Properties").property("ADBE Text Document");
        var d = td.value; d.fontSize = 46; d.applyFill = true; d.fillColor = [1, 1, 1];
        try { d.font = "Arial-BoldMT"; } catch (e) {} td.setValue(d); setPos(txt, [250, 150]); setOp(txt, 85); })();
    var dots = comp.layers.addShape(); dots.name = "DEMO dots";
    (function () { var c = dots.property("ADBE Root Vectors Group").addProperty("ADBE Vector Group").property("ADBE Vectors Group");
        var cols = [[0.2,0.55,1],[0.1,0.82,0.75],[0.62,0.38,1],[1,0.45,0.5],[0.3,0.85,0.4],[1,0.8,0.2]];
        for (var i = 0; i < cols.length; i++) { var g = c.addProperty("ADBE Vector Group").property("ADBE Vectors Group");
            var e = g.addProperty("ADBE Vector Shape - Ellipse"); e.property("ADBE Vector Ellipse Size").setValue([34, 34]);
            e.property("ADBE Vector Ellipse Position").setValue([120 + i * 130, 250]);
            g.addProperty("ADBE Vector Graphic - Fill").property("ADBE Vector Fill Color").setValue(cols[i]); } })();

    // ----- the normal map, moved with the null, used by Displacement Map -----
    var normLayer = comp.layers.add(nrm); normLayer.name = "Glass / Normal (map)";
    follow(normLayer, 0, 0); normLayer.enabled = false;

    // ============================================================
    // GLASS
    // ============================================================
    var shadow = solid("Glass / Drop Shadow", [0, 0, 0]);
    addMask(shadow, roundedRectShape(CX, CY, PW + 6, PH + 6, PR), 26); setOp(shadow, 32); follow(shadow, 0, 16);

    // FROST adjustment: edge refraction (Displacement Map) + body (Spherize) + blur
    var frost = solid("Glass / Frost (adjustment)", [0, 0, 0]); frost.adjustmentLayer = true;
    var fMask = addMask(frost, roundedRectShape(CX, CY, PW, PH, PR), 3);
    setExpr(fMask.property("ADBE Mask Shape"), capsulePathExpr(PW, PH, PR));
    var refractExpr = 'thisComp.layer("Controls").effect("Refract")("Slider")';
    var centerExpr  = 'thisComp.layer("Glass").transform.position';

    // (1) EDGE refraction — Displacement Map driven by the capsule normal map
    var dm = addEffect(frost, "ADBE Displacement Map");
    if (dm) { hasDisp = true; dm.name = "EdgeDisp";
        setP(dm, 2, 1);   // Use For Horizontal = Red
        setP(dm, 4, 2);   // Use For Vertical   = Green
        setExpr(prop(dm, "Max Horizontal Displacement", 3), refractExpr + ' * 0.32');
        setExpr(prop(dm, "Max Vertical Displacement", 5),   refractExpr + ' * 0.32');
        setP(dm, 6, 1);   // Behavior = Center Map
    }

    // (2) BODY distortion — Spherize (centre), cranked
    var sph = addEffect(frost, "ADBE Spherize");
    if (sph) { distortType = "spherize+disp";
        setExpr(prop(sph, "Radius", 1), refractExpr);
        setExpr(prop(sph, "Center of Sphere", 2), centerExpr);
    }

    // (3) frost blur
    var gb = addGauss(frost, null, true);
    setExpr(gb.property(1), 'thisComp.layer("Controls").effect("Frost")("Slider")');
    // (4) vividness
    try { var hs = frost.property("ADBE Effect Parade").addProperty("ADBE HUE SATURATION"); hs.property(4).setValue(12); hs.property(5).setValue(4); } catch (e) {}

    // ----- surface -----
    var tint = solid("Glass / Tint", [1, 1, 1]); addMask(tint, roundedRectShape(CX, CY, PW, PH, PR), 2); setOp(tint, 10); follow(tint, 0, 0);
    var shade = solid("Glass / Shading", [0.5, 0.5, 0.5]); addMask(shade, roundedRectShape(CX, CY, PW, PH, PR), 2);
    addRamp(shade, [CX, pTopY + 4], [0.06, 0.07, 0.06], [CX, pBotY - 4], [0.95, 0.98, 0.95]);
    try { shade.blendingMode = BlendingMode.OVERLAY; } catch (e) {} setOp(shade, 88); follow(shade, 0, 0);
    var topDark = solid("Glass / Top Shade", [0, 0, 0]); addMask(topDark, ellipseShapeObj(CX, pTopY + 4, PW * 0.46, 24), 18); setOp(topDark, 52); follow(topDark, 0, 0);
    var botLight = solid("Glass / Bottom Glow", [1, 1, 1]); addMask(botLight, ellipseShapeObj(CX, pBotY - 5, PW * 0.40, 14), 12);
    try { botLight.blendingMode = BlendingMode.ADD; } catch (e) {} setOp(botLight, 40); follow(botLight, 0, 0);
    var rim = solid("Glass / Rim", [1, 1, 1]); addMask(rim, roundedRectShape(CX, CY, PW, PH, PR), 1.2);
    addMask(rim, roundedRectShape(CX, CY, PW - 3, PH - 3, PR - 1.5), 1.2, MaskMode.SUBTRACT);
    try { rim.blendingMode = BlendingMode.ADD; } catch (e) {} setOp(rim, 48); follow(rim, 0, 0);

    // ----- icons -----
    var ICON = [1, 1, 1], LW = 3.2;
    function vGroup(L) { return L.property("ADBE Root Vectors Group").addProperty("ADBE Vector Group").property("ADBE Vectors Group"); }
    function rectIn(c, w, h, round) { var r = c.addProperty("ADBE Vector Shape - Rect"); r.property("ADBE Vector Rect Size").setValue([w, h]); r.property("ADBE Vector Rect Roundness").setValue(round); }
    function pathIn(c, verts, closed) { var g = c.addProperty("ADBE Vector Shape - Group"); var s = new Shape(); s.vertices = verts; s.closed = !!closed; g.property("ADBE Vector Shape").setValue(s); }
    function ellipseIn(c, dia, off) { var e = c.addProperty("ADBE Vector Shape - Ellipse"); e.property("ADBE Vector Ellipse Size").setValue([dia, dia]); if (off) e.property("ADBE Vector Ellipse Position").setValue(off); }
    function strokeIn(c, col, w) { var s = c.addProperty("ADBE Vector Graphic - Stroke"); s.property("ADBE Vector Stroke Color").setValue(col); s.property("ADBE Vector Stroke Width").setValue(w);
        try { s.property("ADBE Vector Stroke Line Cap").setValue(2); } catch (e) {} try { s.property("ADBE Vector Stroke Line Join").setValue(2); } catch (e2) {} }
    function fillIn(c, col) { c.addProperty("ADBE Vector Graphic - Fill").property("ADBE Vector Fill Color").setValue(col); }
    function iconLayer(name) { var L = comp.layers.addShape(); L.name = name; return L; }
    var add = iconLayer("Glass / Icon Add"); var ca = vGroup(add); rectIn(ca, 52, 42, 11); pathIn(ca, [[-11,0],[11,0]], false); pathIn(ca, [[0,-11],[0,11]], false); strokeIn(ca, ICON, LW); follow(add, -132, 0);
    var bm = iconLayer("Glass / Icon Bookmark"); var cb = vGroup(bm); pathIn(cb, [[-17,-24],[17,-24],[17,24],[0,11],[-17,24]], true); strokeIn(cb, ICON, LW); follow(bm, 0, 0);
    var more = iconLayer("Glass / Icon More"); var cm = vGroup(more); ellipseIn(cm, 6.2, [-16,0]); ellipseIn(cm, 6.2, [0,0]); ellipseIn(cm, 6.2, [16,0]); fillIn(cm, ICON); follow(more, 132, 0);

    // ============================================================
    // finalise — link displacement map layer, tidy stack (fresh by name)
    // ============================================================
    try { comp.layer("Glass").moveToBeginning(); comp.layer("Controls").moveToBeginning(); } catch (e) {}
    if (hasDisp) { try { comp.layer("Glass / Frost (adjustment)").effect("EdgeDisp").property(1).setValue(comp.layer("Glass / Normal (map)").index); } catch (e) {} }
    try { comp.markerProperty.setValueAtTime(0, new MarkerValue("Drag 'Glass'. Refract = distortion. Content goes BELOW 'Glass / Frost'.")); } catch (e) {}

    app.endUndoGroup();

    alert("Liquid Glass Toolbar — build 9 (edge refraction) ✨\n\n" +
          "Edge distortion (Displacement Map): " + (hasDisp ? "ON" : "unavailable") + "\n" +
          "Body distortion (Spherize): " + (distortType ? "ON" : "unavailable") + "\n\n" +
          "• Drag the 'Glass' null. Controls › Refract = distortion amount\n" +
          "  (push it higher for more), Frost = blur, Float = drift.\n" +
          "• To bias more to the EDGE: raise the blur on the 'Hx'/'Hy'\n" +
          "  layers inside the 'Glass Normal' comp (wider edge band).");

})();
